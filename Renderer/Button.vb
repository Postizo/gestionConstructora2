...
#Region "Drawing"
    ...
    Public Shared Sub draw(ByVal g As Graphics, ByVal rect As Rectangle, Optional ByVal theme As Drawing.ColorTheme = Drawing.ColorTheme.Blue, _
        Optional ByVal rounded As Integer = 2, Optional ByVal enabled As Boolean = True, _
        Optional ByVal pressed As Boolean = False, Optional ByVal selected As Boolean = False, _
        ...
        If rect.Width > 2 * rounded And rect.Width > 0 And rect.Height > 2 * rounded And rect.Height > 0 Then
            ...
            If rounded >= 0 Then
                glowPath = Drawing.getGlowingPath(New Rectangle(rect.X + rounded, rect.Y + (0.4 * rect.Height), _
                    rect.Width - (2 * rounded), 0.6 * rect.Height), Drawing.LightingGlowPoint.BottomCenter)
            Else
                glowPath = Drawing.getGlowingPath(New Rectangle(rect.X, rect.Y + (0.4 * rect.Height), _
            ...
            If shadowPath IsNot Nothing Then
                Dim shadowBrush As LinearGradientBrush = New LinearGradientBrush(New Rectangle(rect.X, rect.Y, rect.Width, 5), _
    ...
    ''' <param name="focused">Determine whether the button has input focus.</param>
    Public Shared Sub drawSplit(ByVal g As Graphics, ByVal rect As Rectangle, _
        ByVal split As SplitLocation, ByVal splitSize As Integer, _
        Optional ByVal theme As Drawing.ColorTheme = Drawing.ColorTheme.Blue, _
        Optional ByVal rounded As Integer = 2, Optional ByVal enabled As Boolean = True, _
        Optional ByVal pressed As SplitEffectLocation = SplitEffectLocation.None, _
        Optional ByVal selected As Boolean = False, _
        Optional ByVal hlited As SplitEffectLocation = SplitEffectLocation.None, _
        ...
        Dim btnBrush As LinearGradientBrush = New LinearGradientBrush(rect, Color.Black, Color.White, _
