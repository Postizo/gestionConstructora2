...
#Region "Miscellaneous"
    ...
    End Function
    Public Shared Function combineRect(ByVal rect1 As Rectangle, ByVal rect2 As Rectangle) As Rectangle
        Dim rect3 As Rectangle = rect1
        If rect2.X < rect3.X Then rect3.X = rect2.X
        If rect2.Y < rect3.Y Then rect3.Y = rect2.Y
        ' Combine width and height
        Dim rectRight As Integer = IIf(rect1.Right > rect2.Right, rect1.Right, rect2.Right)
        Dim rectBottom As Integer = IIf(rect1.Bottom > rect2.Bottom, rect1.Bottom, rect2.Bottom)
    ...
    Public Shared ReadOnly Property DisabledTextBrush(Optional ByVal theme As ColorTheme = ColorTheme.Blue) As SolidBrush
        ...
        End Get
    End Property
    Public Shared ReadOnly Property BarGlow() As ColorBlend
        Get
            Dim colors(0 To 3) As Color
            Dim pos(0 To 3) As Single
            Dim glowBlend As ColorBlend = New ColorBlend
    ...
    End Function
    Public Shared Sub drawTextGlow(ByVal g As Graphics, ByVal text As String, ByVal font As Font, _
        ByVal p As PointF, ByVal format As StringFormat, ByVal glow As Color, ByVal count As Integer, _
        ByVal alphaStep As Integer)
        If text = "" Then Return
        If font Is Nothing Then Return
        If count <= 1 Then Return
        If alphaStep <= 0 Or alphaStep > 255 Then Return
        Dim textPath As GraphicsPath = New GraphicsPath
        Dim colorStep As Color = Color.FromArgb(alphaStep, glow.R, glow.G, glow.B)
        Dim penWidth As Integer = count
        textPath.AddString(text, font.FontFamily, CInt(font.Style), font.Size, p, format)
        While penWidth > 1
            Dim penGlow As Pen = New Pen(colorStep, penWidth)
    ...
    End Sub
    Public Shared Sub drawTextGlow(ByVal g As Graphics, ByVal text As String, ByVal font As Font, _
        ByVal area As RectangleF, ByVal format As StringFormat, ByVal glow As Color, ByVal count As Integer, _
        ByVal alphaStep As Integer)
        If text = "" Then Return
        If font Is Nothing Then Return
        If count <= 1 Then Return
        ...
        Dim colorStep As Color = Color.FromArgb(alphaStep, glow.R, glow.G, glow.B)
        Dim penWidth As Integer = count
        Dim emSize As Single = g.MeasureString(text, font).Height * 2 / 3
        textPath.AddString(text, font.FontFamily, CInt(font.Style), emSize, area, format)
        While penWidth > 1
            Dim penGlow As Pen = New Pen(colorStep, penWidth)
            g.DrawPath(penGlow, textPath)
            penWidth = penWidth - 1
